...
truncate() {
  local str="$1"
  local maxlen="$2"
  local ellipsis="…" # This is the literal ellipsis character (U+2026)
...
if [ "$(osascript -e 'if application "Spotify" is running then tell application "Spotify" to get player state')" == "playing" ]; then
  ...
  TRACK=$(truncate "$TRACK" 10)
  ARTIST=$(truncate "$ARTIST" 8)
  sketchybar -m --set $NAME drawing=on
fi
if [ $RUNNING -eq 0 ] && [ $PLAYING -eq 0 ]; then
  sketchybar -m --set spotify_bracket drawing=on
  if [ "$ARTIST" == "" ]; then
  ...
  sketchybar -m --set spotify_bracket drawing=off
